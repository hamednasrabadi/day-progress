// arm64-plugin.js — shrink the sideload APK to arm64-v8a only (~140MB → ~50MB).
// Gated on ARM64_ONLY=1, so it's a NO-OP for any other build — importantly a Play
// Store .aab (production), which must keep all ABIs for Google to split per device.
//
...
// 32-bit-only phones. Every real modern Android device is arm64.
const { withGradleProperties, withAppBuildGradle } = require('@expo/config-plugins');

module.exports = function withArm64Only(config) {
  if (process.env.ARM64_ONLY !== '1') return config;

  // 1) Build only the arm64 native libs (RN / Hermes / reanimated / etc.).
  ...
  // 2) Belt-and-suspenders: filter packaged .so files to arm64, so any dependency
  //    shipping prebuilt all-ABI binaries gets trimmed from the final APK too.
